...
export function summarizeDashboardVisualCompare(
  ...
  const referenceSignals = referencePack.references.flatMap(
    (reference) => reference.signals
  );
  const uniqueSignals = [...new Set(referenceSignals)];
  const matchedSignals = uniqueSignals.filter((signal) => signalMap[signal]);
  const missingSignals = uniqueSignals.filter((signal) => !signalMap[signal]);

  let status: DashboardVisualCompareSummary["status"] = "drifting";
  let label = "Drifting from the reference canon";

  if (matchedSignals.length >= 5) {
    status = "strong";
    label = "Strong reference fit";
  } else if (matchedSignals.length >= 3) {
    status = "mixed";
  ...
  return {
    ...
    matchedSignals,
    missingSignals,
    scoreLabel: `${matchedSignals.length}/${uniqueSignals.length} cues`,
    status,
    summary:
      status === "strong"
        ? `${candidate.blueprint.name} matches most of the visual cues in the ${referencePack.profileLabel} canon.`
        : status === "mixed"
          ? `${candidate.blueprint.name} picks up several ${referencePack.profileLabel} cues, but it still misses some of the profile's visual posture.`
          : `${candidate.blueprint.name} still drifts away from the ${referencePack.profileLabel} canon and needs more of the reference cues.`
  };
}
